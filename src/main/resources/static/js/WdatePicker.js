...
(function() {
	...
	function b() {
		try {
			...
			show : function() {
				...
				this.dd.style.zIndex = J + 2;r(this.dd, "block");r(this.dd.firstChild, "")
			},
			unbind : function(e) {
				e = this.$(e);
				if (e.initcfg) {
	...
	function q() {
		...
		for (var e = 0; e < d.length; e++) {
			...
			I = I.substr(0, I.toLowerCase().indexOf("wdatepicker.js"));var w = I.lastIndexOf("/");
			if (w > 0) {
				I = I.substring(0, w + 1)
			}
			if (I) {
	...
	function F(e) {
		e = e || E;var J = e[z],
			d = J[B],
			I = J.body;
		J = (d && d.scrollTop != null && (d.scrollTop > I.scrollTop || d.scrollLeft > I.scrollLeft)) ? d : I;return {
			top : J.scrollTop,
			left : J.scrollLeft
		}
	}
	...
	function g(M, d) {
		...
		if (d) {
			...
			if (J.eCont) {
				...
				if (J.el == null) {
					alert("WdatePicker:el is null!\nexample:onclick=\"WdatePicker({el:this})\"");return;
				}
				try {
				...
				if (I && J.el.nodeType == 1 && !C(J.el.initcfg, M)) {
					$dp.unbind(J.el);k(J.el, I.type == "focus" ? "onclick" : "onfocus", function() {
						g(M)
					});
					J.el.initcfg = M
				}
	...
	function a(e, d) {
		...
		if (!$dp.dd || e.eCont || ($dp.dd && (e.getRealLang().name != $dp.dd.lang || e.skin != $dp.dd.skin))) {
			if (e.eCont) {
				...
				$dp.dd.style.cssText = "position:absolute";E[z].body.appendChild($dp.dd);w($dp.dd, e);
				if (d) {
		...
		function w(V, P) {
			...
			try {
				...
				S = true;V.removeChild(V.lastChild);var N = E[z].createElement("iframe");
				N.hideFocus = true;
				N.frameBorder = 0;
				N.scrolling = "no";
				N.src = "javascript:(function(){var d=document;d.open();d.domain='" + O + "';})()";V.appendChild(N);setTimeout(function() {
					T = V.lastChild.contentWindow[z];R()
				}, 97);return
			} R();
			function R() {
				var Y = P.getRealLang();
				...
				X.push('<script src="' + u + 'calendar.js"><\/script>');X.push('</head><body leftmargin="0" topmargin="0" tabindex=0></body></html>');X.push('<script>var t;t=t||setInterval(function(){if((typeof(doc.ready)=="boolean"&&doc.ready)||doc.readyState=="complete"){new My97DP();$cfg.onload();$c.autoSize();$cfg.setPos($dp);clearInterval(t);}},20);<\/script>');
				P.setPos = I;
				P.onload = A;T.write("<html>");
				T.cfg = P;T.write(X.join(""));T.close()
			}
		}
		function I(O) {
